...
const Events = (function () {
  ...
  function _browserDownload(title, md) {
    // 开发/浏览器模式：触发浏览器下载（浏览器会让用户选择保存位置或下载到默认目录）
  ...
  function _showExportedToast(info) {
    // info: { path, dir, filename, browserDownload?: true }
    if (info && info.browserDownload) {
      Renderer.showToast('已导出 Markdown，文件已保存到浏览器下载目录');
      return;
    }
    var path = info && info.path ? info.path : '';
    var dir = info && info.dir ? info.dir : '';
    var filename = info && info.filename ? info.filename : '';
    var msg;
    if (path) {
      msg = '已保存：' + path;
    } else {
      msg = '已导出 Markdown：' + filename;
    }
    Renderer.showToast(msg, {
      duration: 18000,
      actionText: '打开所在文件夹',
      actionFn: function () {
        if (window.pywebview && window.pywebview.api && typeof window.pywebview.api.open_folder_in_explorer === 'function') {
          window.pywebview.api.open_folder_in_explorer(dir || path).then(function (res) {
            if (!res || !res.ok) {
              _fallbackCopyPath(path);
            }
          }).catch(function (e) {
            _fallbackCopyPath(path);
          });
  ...
  function _fallbackCopyPath(path) {
    if (!path) {
      Renderer.showToast('未获取到保存路径');
      return;
    }
    try {
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(path).then(function () {
          Renderer.showToast('路径已复制：' + path);
        }).catch(function () {
          Renderer.showToast('保存位置：' + path, { duration: 15000 });
        });
      } else {
        Renderer.showToast('保存位置：' + path, { duration: 15000 });
      }
  ...
  async function exportMarkdown(b) {
    var title = b.title || 'bookmark';
    var md = b.markdown || ('# ' + title + '\n\n暂无正文内容');

    // 优先：桌面端 pywebview 原生保存对话框 —— 让用户自由选择保存路径
    var pywebviewFailed = false;
    if (window.pywebview && window.pywebview.api && typeof window.pywebview.api.save_markdown_dialog === 'function') {
      try {
        var r = await window.pywebview.api.save_markdown_dialog(title, md);
        if (r && r.ok) {
          _showExportedToast(r);
          return;
        } else if (r && r.canceled) {
          // 用户主动取消，不提示，也不降级
          return;
        }
        // 其他情况（r.error、r=null、未知异常）：继续降级
        if (r && r.error) {
          pywebviewFailed = true;
        } else {
          pywebviewFailed = true;
        }
      } catch (e) {
    ...
    // 其次：后端 API 写入桌面/导出目录，写入成功后 Toast 必须显示完整路径
    try {
      var r2 = await StorageManager.exportMarkdownFile(b);
      if (r2 && r2.ok) {
        _showExportedToast(r2);
        return;
      }
    } catch (_) {}

    // 兜底：浏览器触发下载
    _browserDownload(title, md);
    _showExportedToast({ browserDownload: true });
  }
