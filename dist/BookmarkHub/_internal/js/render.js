...
const Renderer = (function () {
  ...
  function renderCards() {
    ...
    grid.innerHTML = list.map(function (b, i) {
      ...
      // 方案C：正文极薄（空或 <80字）时，卡片摘要末尾追加提示，避免用户误以为有完整正文
      var isThinContent = !b.markdown || b.markdown.length < 80;
      if (isThinContent && excerpt) {
        excerpt += ' · 仅摘要';
      }
  ...
  function showToast(msg, options) {
    ...
    if (opts.actionText && opts.actionFn) {
      ...
      a.onclick = function () {
        try { opts.actionFn(); } catch (e) { console.error(e); }
        // 点击后保持显示一段时间，不立即关闭
      };
      toast.appendChild(a);
    }
