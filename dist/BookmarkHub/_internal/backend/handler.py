...
from .db import PROJECT_ROOT, DATA_DIR
...
def _get_export_dir():
    """Markdown 导出目录：优先桌面，其次用户目录下的书签集导出。"""
    ...
    if os.path.isdir(desktop):
        ...
        base = home
    d = os.path.join(base, 'BookmarkHub-Export')
...
def export_markdown_to_file(bookmark):
    """将单个书签的 Markdown 写入文件，返回绝对路径。"""
    exp_dir = _get_export_dir()
    fname = _sanitize_filename(bookmark.get('title') or '') + '.md'
    fpath = os.path.join(exp_dir, fname)
    # 同名冲突时追加序号
    if os.path.exists(fpath):
        base, ext = os.path.splitext(fname)
        for i in range(1, 999):
            cand = os.path.join(exp_dir, '{} ({}){}'.format(base, i, ext))
            if not os.path.exists(cand):
                fpath = cand
                break
    md = bookmark.get('markdown') or ('# ' + (bookmark.get('title') or '') + '\n\n暂无正文内容')
    ...
    def _route_post(self, path):
        ...
        try:
            ...
            if path == '/api/export/markdown':
                """将书签 Markdown 写入本地文件（桌面/BookmarkHub-Export/），返回路径。
                Body: { id: number } 或 { title: string, markdown: string }
                """
                body = self._read_body()
                bid = body.get('id')
                bm = None
                ...
                try:
                    fpath = export_markdown_to_file(bm)
                except OSError as e:
                    return self._send_json(500, {'error': '写入失败：' + str(e)})
                return self._send_json(200, {
                    'ok': True,
                    'path': fpath,
