...
def _format_bilibili(data, base_url):
    ...
    # description：优先用视频简介，为空时用元信息生成摘要，避免卡片空白
    if desc:
        description = desc[:200]
    else:
        desc_parts = []
        if author:
            desc_parts.append('UP主: ' + author)
        if tname:
            desc_parts.append('分区: ' + tname)
        if date_str:
            desc_parts.append('发布: ' + date_str)
        if stat_parts:
            desc_parts.append(' | '.join(stat_parts))
        description = ' · '.join(desc_parts)[:200] if desc_parts else 'B站视频'
...
def _format_douyin(data, base_url):
    ...
    # description：优先用视频描述，为空时用元信息生成摘要，避免卡片空白
    if desc:
        description = desc[:200]
    else:
        desc_parts = []
        if author:
            desc_parts.append('作者: ' + author)
        if date_str:
            desc_parts.append('发布: ' + date_str)
        if stat_parts:
            desc_parts.append(' | '.join(stat_parts))
        if tags:
            desc_parts.append('话题: ' + ' '.join('#' + t for t in tags))
        description = ' · '.join(desc_parts)[:200] if desc_parts else '抖音视频'
...
def _extract_meta_fallback(html, base_url, default_title):
    ...
    # 兜底：markdown 极短时补充提示
    if len(md.strip()) < 50:
        md = '# {}\n\n> 视频内容无法提取，仅保存元信息。请点击「原网页打开」观看视频。'.format(title or default_title)
    if not description:
        description = default_title + ' · 视频内容无法提取，仅保存元信息'
    return title, description, md
